...
DEFAULT_XAPI_STREAMING_PORT_DEMO = 5125

DEFAULT_XAPI_PORT_REAL           = 5112
DEFAULT_XAPI_STREAMING_PORT_REAL = 5113
...
class JsonSocket(object):
    ...
    def _read(self, bytesSize=32096):
        if not self.socket:
...
class APIClient(JsonSocket):
    def __init__(self, address=DEFAULT_XAPI_ADDRESS, encrypt=True, is_real=True):
        if is_real:
...
class APIStreamClient(JsonSocket):
    def __init__(self, address=DEFAULT_XAPI_ADDRESS, is_real=True, encrypt=True, ssId=None, 
                 tickFun=None, tradeFun=None, balanceFun=None, tradeStatusFun=None, profitFun=None, newsFun=None):
        
        if is_real:
            port = DEFAULT_XAPI_STREAMING_PORT_REAL
        else:
            port = DEFAULT_XAPI_STREAMING_PORT_DEMO
